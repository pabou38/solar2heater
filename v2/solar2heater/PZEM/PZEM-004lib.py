...
class PZEM:
	...
	def __init__(self, com="COM3", timeout=10.0):
	
		# PZEM 8bits, 1 stop, no parity
	...
	def isReady(self):
		print('is ready. writing')
		self.ser.write(serial.to_bytes(self.setAddrBytes)) # Convert a sequence to a bytes type.
		print('is ready. reading 7 bytes')

		rcv = self.ser.read(7)
		print(len(rcv), rcv)

		if len(rcv) == 7:
			# struct Interpret bytes as packed binary data . handling binary data stored in files, database or from network connections 
			# unpacks the packed value into its original representation with the specified format. This function always returns a tuple, even if there is only one element. 
			"""
			?: boolean
			h: short
			l: long
